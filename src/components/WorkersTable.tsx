...
import { MoreHorizontal, FileText, BadgeCheck, AlertTriangle } from 'lucide-react';

export default function WorkersTable() {
  const workers = [
    { name: 'Sarah Chen', cos: 'C4X-992-L', status: 'Compliant', expiry: '12 Oct 2026', type: 'Skilled Worker' },
    { name: 'Marcus Rashford', cos: 'A01-882-K', status: 'Expiring Soon', expiry: '02 May 2026', type: 'Senior Talent' },
            ...
            <tr key={worker.cos} className="group hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
              ...
              <td className="p-4">
                <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ${
                  worker.status === 'Compliant' ? 'bg-teal-50 text-teal-700 border border-teal-100 dark:bg-teal-900/40 dark:text-teal-400 dark:border-teal-800' : 'bg-amber-50 text-amber-700 border border-amber-100 dark:bg-amber-900/40 dark:text-amber-400 dark:border-amber-800'
                }`}>
